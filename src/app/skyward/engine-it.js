// Generated from Skyward Mappa IT.dc.html by the Skyward port script. Do not edit by hand.
/* eslint-disable */
export function createEngine(React, DCLogic, __PL, __send) {

const SYS=[
{id:'work',code:'SYS-01',name:'Lavori',min:4,desc:'Progetti selezionati, a partire da Skyward stesso.',meta:'1 pianeta',hex:'#7FE6F2',tint:'127,230,242',core:12,p:[380,40,-210]},
{id:'services',code:'SYS-02',name:'Servizi',min:3,desc:'Web design, UI/UX, sviluppo e design system.',meta:'4 pianeti',hex:'#FFD27A',tint:'255,210,122',core:11,p:[-480,-25,-40]},
{id:'process',code:'SYS-03',name:'Processo',min:2,desc:'Quattro fasi, dalla prima call al lancio.',meta:'4 fasi',hex:'#FF7A6B',tint:'255,122,107',core:10,p:[560,60,320]},
{id:'about',code:'SYS-04',name:'Chi sono',min:2,desc:'Chi sono e come lavoro.',meta:'3 pianeti',hex:'#8CF0A0',tint:'140,240,160',core:10,p:[30,-40,500]},
{id:'contact',code:'SYS-05',name:'Contatti',min:1,desc:'Raccontami il tuo progetto.',meta:'2 pianeti',hex:'#C3A6FF',tint:'195,166,255',core:9,p:[-240,70,-560]},
{id:'home',code:'SYS-00',name:'Home',min:1,desc:'Dove iniziano tutte le rotte.',meta:'pianeta casa',hex:'#FFB38A',tint:'255,179,138',core:10,p:[-40,0,90]}
];
const HOME=5;
...
class Component extends DCLogic {
  ...
  shot=(()=>{try{const m=/[?&]shot=([\w-]+)/.exec(location.search);return m?m[1]:null;}catch(e){return null;}})();
  get reduced(){return !!this.shot||!!this.props.forceReducedMotion||(typeof matchMedia!=='undefined'&&matchMedia('(prefers-reduced-motion: reduce)').matches);}
  ...
  componentDidMount(){
    ...
    const qp=this.shot?'high':(this.props.quality??'auto'),low=innerWidth<760||(navigator.hardwareConcurrency||8)<=4;this.setQ(qp==='high'?1:qp==='low'?.4:(low?.7:1));
    ...
    if(dl.bad){this.setState({lost:'/'+dl.bad,live:'Pagina non trovata.'});}
    else if(dl.i>=0)this.openDirect(dl.i,dl.j);
    else if(this.wantIntro()){this.revealAt=Infinity;this.setState({intro:'boot'});this.introT=setTimeout(()=>this.setState(st=>st.intro==='boot'?{intro:'ready',live:'Pronto. Premi il pulsante per partire.'}:null),this.reduced?300:1900);}
  ...
  runShot(){const s=this.shot,q=new URLSearchParams(location.search),T=(fn,ms)=>setTimeout(fn,ms);window.__skyReady=false;
    if(s==='coach')T(()=>this.coachGo(0),500);
    if(s==='card')T(()=>this.select(+(q.get('sys')||0)),500);
    if(s==='list')T(()=>this.openList(),500);
    if(s==='formerr')T(()=>{this.fSubmit({preventDefault(){}});const a=document.activeElement;if(a&&a.blur)a.blur();},900);
    if(s==='formsent')T(()=>this.setState({fstate:'sent'}),900);
    T(()=>{try{let t=performance.now();for(let k=0;k<90;k++){t+=16.7;this.frame(t);}}catch(e){console.warn(e);}window.__skyReady=true;},s==='intro'?2600:3200);}
  ...
  perf(t){if(this.shot||(this.props.quality??'auto')!=='auto'){this.pt=t;return;}const d=this.pt?t-this.pt:16;this.pt=t;if(d>0&&d<250)this.ema=this.ema?this.ema*.94+d*.06:d;
  ...
  wantIntro(){if(this.shot)return this.shot==='intro';if(this.langIn)return false;if(/[?&]return=1/.test(location.search))return false;const m=this.props.intro??'always';if(m==='never')return false;if(m==='always')return true;try{return localStorage.getItem('skyward.introSeen')!=='1';}catch(e){return true;}}
  wantCoach(){if(this.shot)return false;if(this.langIn)return false;if(/[?&]return=1/.test(location.search))return false;if((this.props.intro??'always')==='always')return true;try{return localStorage.getItem('skyward.coachSeen')!=='1';}catch(e){return true;}}
  ...
  parseHash(){let h='';try{h=decodeURIComponent((location.hash||'').replace(/^#\/?/,''));}catch(e){}const [a,b]=h.split('/');const i=SYS.findIndex(s=>s.id===a);if(i<0||i===HOME)return{i:-1,j:-1,bad:!!a&&a!=='home'?h:null};let j=-1;if(b)j=(PL[a]||[]).findIndex(p=>!p.ghost&&(p.key||slug(p.name))===b);return{i,j,bad:b&&j<0?h:null};}
  ...
  lostHome=()=>{this.lostTrap(false);this.setState({lost:null,live:'Di nuovo sulla mappa della galassia.'});try{history.replaceState(null,'','#/');}catch(e){}if(this.state.phase!=='map'){this.goHome();}else setTimeout(()=>this.startCoach(),900);};
  hashFor(){const st=this.state;if(st.intro||this.warp||this.egg)return null;if(st.phase==='map')return '#/';if(st.sel<0)return null;const id=SYS[st.sel].id;if(st.phase==='arrive')return '#/'+id;if(st.phase==='page'){const p=(PL[id]||[])[st.psel];return p?'#/'+id+'/'+(p.key||slug(p.name)):'#/'+id;}return null;}
  ...
  openPage(j,anim){const st=this.state,p=(PL[SYS[st.sel].id]||[])[j];if(!p||p.ghost)return;
    const go=()=>{this.setState({phase:'page',psel:j,landMsg:'',live:'Atterraggio su '+p.name+'.'},()=>{if(this.pageEl)this.pageEl.scrollTop=0;setTimeout(()=>this.pageTitle&&this.pageTitle.focus({preventScroll:true}),80);});};
  ...
  fCheck(n,v){v=(v||'').trim();if(n==='nome')return v?'':'Inserisci il tuo nome.';if(n==='email'){if(!v)return 'Inserisci la tua email.';return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v)?'':'Controlla l’email, es. nome@studio.com';}if(n==='msg')return v.length>=10?'':'Raccontami in poche righe di cosa hai bisogno.';return '';}
  ...
  fSubmit=e=>{e.preventDefault();if(this.state.fstate==='sending')return;const fv=this.state.fv,er={};['nome','email','msg'].forEach(n=>{er[n]=this.fCheck(n,fv[n]);});this.setState({ferr:er});const bad=['nome','email','msg'].find(n=>er[n]);if(bad){const el=document.getElementById('f-'+bad);el&&el.focus();return;}
    this.setState({fstate:'sending',live:'Invio in corso.'});__send(fv,'it').then(ok=>this.setState(ok?{fstate:'sent',fv:{nome:'',email:'',msg:''},ferr:{},live:'Messaggio inviato.'}:{fstate:'idle',live:'Invio non riuscito. Riprova o scrivimi via email.'}));};
  ...
  drawMiniJump(m,t,dt){const c=m.ctx,W=m.W,H=m.H;c.setTransform(m.d,0,0,m.d,0,0);const R0=m.run;let v=.03,k=0;
    ...
    if(R0){const q=clamp((k-.6)/.4,0,1),e=q*q*(3-2*q),out=k>1?clamp((k-1)*R0.dur/1500,0,1):0,fade=out>.7?1-(out-.7)/.3:1,S=6+e*Math.min(W,H)*.3;c.globalAlpha=(.25+.75*e)*fade;const h=S*2.4;c.drawImage(this.sprite(R0.tint,true),cx-h/2,cy-h/2,h,h);
      ...
      c.globalCompositeOperation='source-over';c.globalAlpha=clamp(k/.12,0,1)*fade;c.textAlign='center';c.fillStyle='#F2EEE6';c.font='500 13px "Martian Mono", monospace';c.fillText((k<1?'SALTO · ':'ARRIVAL · ')+R0.name.toUpperCase(),W/2,H-26);c.textAlign='start';}
    else{c.globalCompositeOperation='source-over';c.globalAlpha=.9;c.textAlign='center';c.fillStyle='#A9A3C2';c.font='400 12px "Martian Mono", monospace';c.fillText('IN ATTESA DI ROTTA',W/2,H-26);c.textAlign='start';}
    c.globalAlpha=1;}
  jumpDemo=()=>{const m=this.minis&&this.minis.jump;if(!m||m.run)return;const s=SYS[this.state.miniSel];m.parts.forEach(q=>Object.assign(q,this.spawn(true)));m.run={start:performance.now(),dur:this.reduced?1:2200,tint:s.tint,name:s.name};this.setState({jumpBusy:true,live:'Salto dimostrativo verso '+s.name+'.'});this.sfx('jump',2.2);};
  closePage=()=>{const i=this.state.sel;this.setState({phase:'arrive',live:'Sistema '+(SYS[i]?SYS[i].name:'')+'.'},()=>setTimeout(()=>this.arrive&&this.arrive.focus({preventScroll:true}),40));};
  ...
  bhClick=()=>{if(this.state.phase!=='map'||this.egg||this.state.intro)return;
    if(this.reduced){this.setState({eggMsg:'Un buco nero. Meglio stare alla larga.'});clearTimeout(this.eggT);this.eggT=setTimeout(()=>this.setState({eggMsg:''}),4000);return;}
    this.egg={start:performance.now()};this.hold=null;this.vel=0;this.tgt.T=[...this.bh.p];this.tgt.dist=380;this.tgt.yaw=null;this.setState({sel:-1,hover:-1,eggMsg:'',live:'Risucchio in corso.'});this.sfx('egg',2.6);};
  drawEgg(c,W,H,t){const e=this.egg;if(!e)return;const k=(t-e.start)/2800,b=this.bhScr||{x:W/2,y:H/2,R:30};c.globalCompositeOperation='source-over';
    if(k<.45){const q=k/.45,rad=Math.max(W,H)*(1.25-q*1.05),g=c.createRadialGradient(b.x,b.y,Math.max(1,rad*.2),b.x,b.y,rad);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(0,0,0,'+(.2+q*.8)+')');c.globalAlpha=1;c.fillStyle=g;c.fillRect(0,0,W,H);}
      ...
      c.globalCompositeOperation='source-over';c.globalAlpha=1;c.fillStyle='#000';c.beginPath();c.arc(cx,cy,30+q*50,0,6.283);c.fill();c.strokeStyle='rgba(255,230,200,.9)';c.lineWidth=1.5;c.beginPath();c.arc(cx,cy,32+q*50,0,6.283);c.stroke();}
        ...
        this.setState({here:HOME,eggMsg:'Oltre l’orizzonte degli eventi, e ritorno.',live:'Sei di nuovo a Home.'});clearTimeout(this.eggT);this.eggT=setTimeout(()=>this.setState({eggMsg:''}),4200);}
  ...
  drawWarp(c,W,H,t,dt){
    ...
    if(w.name){const ha=clamp(k/.15,0,1)*clamp((1-k)/.12,0,1)*fo,pe=k<.5?2*k*k:1-Math.pow(-2*k+2,2)/2,dist=(w.ly*(1-pe)).toFixed(1),y0=H-Math.max(40,H*.08);
      c.globalAlpha=ha;c.textAlign='center';c.fillStyle='#F2EEE6';c.font='500 13px "Martian Mono", monospace';c.fillText('SALTO · '+w.name.toUpperCase(),W/2,y0-22);
      c.fillStyle='#A9A3C2';c.font='400 12px "Martian Mono", monospace';c.fillText('DISTANZA '+dist+' AL',W/2,y0);
  ...
  select(i){this.fly(i);if(i>=0)this.sfx('blip');this.setState({sel:i,live:i>=0?SYS[i].name+', '+SYS[i].code+', distanza '+SYS[i].min+' minuti. Invio per saltare.':''});}
  ...
  recenter=()=>{this.fly(-1);this.setState({sel:-1,live:'Mappa ricentrata.'});};
  zoomIn=()=>{this.zoomBy(.7);this.idle=performance.now();};
  zoomOut=()=>{this.zoomBy(1/.7);this.idle=performance.now();};
  openList=()=>this.setState({list:true,sel:-1,live:'Vista a elenco. M per tornare alla mappa.'});
  openMap=()=>this.setState({list:false,live:'Vista mappa.'});
  ...
  jump=()=>{
    ...
    this.setState({list:false,phase:'jump',live:'Salto verso '+s.name+'.'});
  ...
  goHome=()=>{
    const st=this.state;
    const land=()=>{this.setState({phase:'map',here:HOME,sel:-1,list:false,live:'Home. Mappa di partenza.'},()=>{this.fly(-1);this.tgt.yaw=.7+Math.round((this.cam.yaw-.7)/6.2832)*6.2832;this.tgt.pitch=.52;this.tgt.dist=DHOME;this.tgt.T=[0,0,0];});};
    if(st.here===HOME||this.reduced){land();return;}
    const p=(this.scr&&this.scr[HOME])||[this.W/2,this.H/2];
    this.setState({sel:HOME,list:false,phase:'jump',live:'Ritorno a Home.'});
    this.warpP.forEach(q=>Object.assign(q,this.spawn(true)));
  ...
  arriveNow(){const i=this.state.sel;this.orb=[];this.orb0=null;this.ospd=1;this.setState({psel:-1,phover:-1,landMsg:''});this.tgt.dist=220;this.cam.dist=this.reduced?220:45;if(!this.reduced){this.emerge={start:performance.now(),s0:Math.max(1,this.emergeScale||6)};}this.arriveAnimVal=this.emerge?'none':'skFade 420ms ease-out';this.emergeScale=0;this.setState(st=>({phase:'arrive',visited:{...st.visited,[i]:true},live:'Arrivo a '+SYS[i].name+'.'}),()=>setTimeout(()=>this.arrive&&this.arrive.focus(),30));}
  back=()=>{const i=this.state.sel;this.setState({psel:-1,phover:-1,landMsg:'',phase:'map',here:i,sel:-1,live:'Mappa. Sei qui: '+SYS[i].name+'.'},()=>{this.fly(-1);setTimeout(()=>this.btns[i]&&this.btns[i].focus(),30);});};
  ...
  renderVals(){
    ...
    const systems=SYS.map((s,i)=>{
      ...
      return{
        ...
        hud:s.code+' · '+s.min+' min'+(vis?' · visitato':''),
        aria:s.name+(isHere?', sei qui':'')+', '+s.desc+' Distanza '+s.min+' minuti'+(vis?', visitato':''),
        listMeta:isHere?'sei qui':s.meta+' · '+s.min+' min'+(vis?' · visitato':''),
    ...
    if((phase==='arrive'||phase==='page')&&sel>=0){
      ...
      arrive={planets,orbitsD:od,ghostD:gd,orbitSelD:ps>=0?ell(rx[ps]):'',mapCx:cx,mapCy:cy,mapCxPct:(cx/vw*100).toFixed(1)+'%',mapCyPct:(cy/vh*100).toFixed(1)+'%',
        ...
        planetOnMob:!!P0,planetOnDesk:false,pl:P0?{...P0,cta:P0.ghost?'Salta a Contatti':'Atterra su '+P0.name}:{},
    ...
    const CS=[{t:'Trascina per ruotare',d:mob?'Trascina con un dito per far girare la galassia.':'Tieni premuto il tasto del mouse e trascina per far girare la galassia.'},{t:'Ingrandisci',d:mob?'Pizzica con due dita per ingrandire.':'Usa la rotella o i tasti + e − per ingrandire.'},{t:'Entra in un sistema',d:mob?'Tocca un sistema per l’anteprima. Tieni premuto per saltarci dentro.':'Passa sopra un sistema per l’anteprima. Tieni premuto per saltarci dentro.'}];
    let coachIcon=null;
    ...
    if(phase==='page'&&sel>=0){const sy=SYS[sel],L0=PL[sy.id]||[],j=Math.max(0,this.state.psel),p=L0[j]||L0[0],prev=j>0?L0[j-1]:null,nx=L0[j+1],hasNext=!!nx&&!nx.ghost,isC=sy.id==='contact',ix=n=>String(n).padStart(2,'0'),c=p.c||PC.ice;
      const tp=p.type||'about',L1=L0.filter(q=>!q.ghost);page={isCase:tp==='case',isService:tp==='service',isStep:tp==='step',isCrew:tp==='crew',isAbout:tp==='about',isContact:tp==='contact',isForm:tp==='form',showIntro:tp!=='form',
        ...
        hasPrev:!!prev&&!prev.ghost,prevIdx:prev?ix(j):'',prevName:prev?prev.name:'',nextIdx:hasNext?ix(j+2):(isC?'Galassia':'SYS-05'),nextName:hasNext?nx.name:(isC?'Mappa della galassia':'Contatti')};
      Object.assign(page,{hasPhoto:tp==='about'&&['me','experience','approach'].includes(p.key),photoHex:c[1],phMe:p.key==='me',phExp:p.key==='experience',phAppr:p.key==='approach',photoSrc:{experience:'assets/about/cristian-laptop.png',approach:'assets/about/cristian-idea.png'}[p.key]||'assets/about/cristian.png',photoAlt:{experience:'Memoji di Cristian: capelli castani, occhiali tondi, sbuca da dietro un laptop',approach:'Memoji di Cristian con il dito alzato e una lampadina sopra la testa'}[p.key]||'Memoji di Cristian che saluta: capelli castani, occhiali tondi, maglione nero',showIntro:!!p.lead,lead:p.lead||'',body:p.body||'',hasBody:!!p.body,items:p.items||[],happens:p.happens||'',gets:p.gets||[],blocks:p.blocks||[]});
    ...
    const extra={shotOn:!!this.state.shot,shotSrc:this.state.shot?this.state.shot.src:'',shotAlt:this.state.shot?this.state.shot.alt:'',openShot:e=>{const d=e.currentTarget.dataset;this.shotFrom=e.currentTarget;this.setState({shot:{src:d.src,alt:d.alt}});},closeShot:e=>{if(e)e.stopPropagation();this.setState({shot:null});const b=this.shotFrom;if(b)setTimeout(()=>b.focus({preventScroll:true}),30);},shotImgRef:el=>{if(el&&this.state.shot&&el.getAttribute('src')!==this.state.shot.src)el.setAttribute('src',this.state.shot.src);},shotCloseRef:el=>{if(el&&el!==this.shotBtn){this.shotBtn=el;setTimeout(()=>el.focus(),40);}if(!el)this.shotBtn=null;},lostBtnRef:el=>{if(el&&el!==this.lostBtn){this.lostBtn=el;setTimeout(()=>{this.lostTrap(true);el.focus();},60);}if(!el&&this.lostBtn){this.lostBtn=null;this.lostTrap(false);}},kbdDisp:coarse?'none':'inline',lostOn:!!this.state.lost,lostPath:this.state.lost||'',lostHome:this.lostHome,lostJumps:SYS.filter(s=>s.id!=='home').map(s=>({name:s.name,hex:s.hex,code:s.code,go:()=>{const i=SYS.indexOf(s);this.lostTrap(false);this.setState({lost:null});try{history.replaceState(null,'','#/'+s.id);}catch(e){}this.openDirect(i,-1);}})),showWordmark:!mob,showRecenterLabel:!mob,crumbDisplay:mob?'none':'flex',
      ...
      audioOn:audio,audioAria:audio?'Audio attivo, disattiva':'Audio disattivato, attiva',audioWave:audio?'M13 7.5a3.5 3.5 0 0 1 0 5M15.2 5.2a6.6 6.6 0 0 1 0 9.6':'M13 8l4 4M17 8l-4 4',toggleAudio:this.toggleAudio,
      introOn:!!intro,introReady:intro==='ready',introBg:intro==='warp'?'rgba(7,6,15,0)':'#07060F',introContentOp:intro==='warp'?0:1,introContentTf:intro==='warp'?'scale(1.12)':'none',showSkip:intro==='boot'||intro==='ready',introHint:'TIENI PREMUTO PER PARTIRE',
      ...
      coachOn:coach>=0&&phase==='map'&&!intro&&!list&&this.cardIdx()<0,coachCount:(coach+1)+' / 3',coachTitle:coach>=0?CS[coach].t:'',coachText:coach>=0?CS[coach].d:'',coachIcon,coachNextLabel:coach<2?'Avanti':'Inizia',
      ...
      fv:this.state.fv,fIn:this.fIn,fBlur:this.fBlur,fSubmit:this.fSubmit,fSending:this.state.fstate==='sending',fSent:this.state.fstate==='sent',fBtn:this.state.fstate==='sending'?'Invio…':'Invia messaggio',fBtnOp:this.state.fstate==='sending'?.7:1,
      fe:Object.fromEntries(['nome','email','msg'].map(n=>{const m=this.state.ferr[n];const help={nome:'',email:'Ti risponderò a questo indirizzo.',msg:'Bastano poche righe: obiettivo, tempi, budget indicativo.'}[n];return[n,{inv:!!m,msg:m||help,color:m?'#FF8FB1':'#A9A3C2',border:m?'1.5px solid #FF8FB1':'1px solid rgba(242,238,230,.35)'}];})),
      logoDemo:(()=>{const lp=this.state.logoPlay,hx=SYS[1].hex;return ce('svg',{key:'lg'+lp,viewBox:'0 0 1000 1000',role:'img','aria-label':'Logo di Skyward',style:{width:'100%',height:'100%'}},ce('g',{transform:'matrix(1.577362,0,0,1.577362,-316.787463,-268.230134)',fill:'#F2EEE6',fillOpacity:0,stroke:hx,strokeWidth:5,strokeLinejoin:'round'},LOGO.map((d,i)=>ce('path',{key:i,d,pathLength:1,style:{strokeDasharray:1,strokeDashoffset:1,animation:'skDraw 1800ms '+(i*180)+'ms cubic-bezier(.65,0,.25,1) forwards, skFillIn 600ms '+(1700+i*180)+'ms ease-out forwards'}}))));})(),
      ...
      mSelHex:SYS[this.state.miniSel].hex,jumpDemo:this.jumpDemo,jumpBusy:this.state.jumpBusy,jumpOp:this.state.jumpBusy?.6:1,jumpLabel:this.state.jumpBusy?'Salto in corso…':'Avvia il salto verso '+SYS[this.state.miniSel].name,
    ...
    return{
      ...
      listOn:list,listLabel:mob?(list?'Mappa':'Elenco'):(list?'Vista mappa':'Vista a elenco'),listKey:mob?'':(list?'M':'L'),
      ...
      heroEyebrow:here===HOME?'Skyward · Design e sviluppo':'Sei qui · '+SYS[here].name,
      sel:{...S,cta:ci===HOME?'Torna alla Home':ci===here?'Entra in '+S.name:'Salta a '+S.name,route:ci===here?'Posizione attuale':'Rotta · '+SYS[here].name+' → '+S.name,visitLabel:ci===here?'sei qui':vis?'visited':'non visitato',visitColor:vis||ci===here?S.hex:'#A9A3C2',visitBorder:vis?'rgba('+S.tint+',.55)':'rgba(242,238,230,.22)'}
    };
  }
}
