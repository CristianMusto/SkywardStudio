...
import type { VNode } from '../engine/vnode';

const SVG_NS = 'http://www.w3.org/2000/svg';
/** Numeric style values that must NOT get a 'px' suffix (same list React uses, plus SVG stroke props). */
...
/** SVG attributes that are camelCase in the spec and must not be kebab-cased. */
...
function build(n: any, svg = false): Node {
  ...
  if (Array.isArray(n)) {
    ...
    n.forEach(c => f.appendChild(build(c, svg)));
    return f;
  }
    ...
    if (k === 'style') {
      for (const [sk, sv] of Object.entries(v as Record<string, any>)) {
        ...
        (el as HTMLElement).style.setProperty(
          prop,
          typeof sv === 'number' && !UNITLESS.has(sk) ? sv + 'px' : String(sv),
        );
      }
      continue;
    }
